...
version = "0.1.0"
author = "org.gavr.sas"
description = "This is break out pixel game in nico framework"
license = "?"
...
task runr, "Runs nimout for current platform":
 exec &"nim c -r {releaseOpts} -o:nimout src/main.nim"

task rund, "Runs debug nimout for current platform":
 exec &"nim c -r {debugOpts} -o:nimout src/main.nim"

task release, "Builds nimout for current platform":
 exec &"nim c {releaseOpts} -o:nimout src/main.nim"

task webd, "Builds debug nimout for web":
 exec &"nim c {debugOpts} -d:emscripten -o:nimout.html src/main.nim"

task webr, "Builds release nimout for web":
 exec &"nim c {releaseOpts} -d:emscripten -o:nimout.html src/main.nim"

task debug, "Builds debug nimout for current platform":
 exec &"nim c {debugOpts} -o:nimout_debug src/main.nim"
